...
def update_memory(memory_id, text, date, gpt_analysis, image_url):
    """기억 수정"""
    conn = get_connection()
    if conn is None:
        logging.error("DB 연결 실패: update_memory 수행 불가")
        return False
    ...
    try:
        ...
        SET text=%s, date=%s, gpt_analysis=%s, image_url=%s
        WHERE id=%s
        """
        gpt_json = json.dumps(gpt_analysis, ensure_ascii=False) if gpt_analysis else None
        cursor.execute(sql, (text, date, gpt_json, image_url, memory_id))
        conn.commit()
        return cursor.rowcount > 0
    except Exception as e:
        logging.error(f"기억 수정 실패: {e}")
        return False
    finally:
        if cursor:
            cursor.close()
...
def delete_memory(memory_id):
    ...
    conn = get_connection()
    if conn is None:
        logging.error("DB 연결 실패: delete_memory 수행 불가")
        return False

    cursor = None
    try:
        cursor = conn.cursor()
        sql = "DELETE FROM memory_archive WHERE id = %s"
        cursor.execute(sql, (memory_id,))
        conn.commit()
        return cursor.rowcount > 0
    except Exception as e:
        logging.error(f"기억 삭제 실패: {e}")
        return False
    finally:
        if cursor:
            cursor.close()
